...
- **Weekly execution is best-effort, not guaranteed.** GitHub may delay or drop a scheduled run under load, and separately, **GitHub disables scheduled workflows on a public repository after 60 days with no repository activity.** GitHub's own docs state the 60-day rule without defining "activity"; cross-referencing multiple independent GitHub community sources on this behavior in practice (not one official statement, so corroborated rather than primary-confirmed) is consistent that activity means a push/commit, a release, or a merged PR — explicitly not read-only actions like browsing files or viewing the repo. This workflow commits `state/last-check.json` on every run regardless of outcome (see Output below) — a genuine weekly push — which should keep the schedule from ever being disabled on inactivity grounds. Manually opening or browsing this repo would not have achieved the same thing even if attempted.
